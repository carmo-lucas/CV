\documentclass[a4paper, 11pt]{article}
\usepackage[dvipsnames]{xcolor}

\definecolor{Accent}{RGB}{6, 28, 68}
\definecolor{Accent2}{RGB}{80,80,80}
\usepackage[utf8]{inputenc}
\usepackage[colorlinks=true,urlcolor=Accent]{hyperref}
\usepackage{titlesec}
\usepackage[margin=1.5cm]{geometry}
\usepackage{longtable}
\usepackage{graphicx}
\usepackage{colortbl}
\usepackage{makecell}
\usepackage{fancyhdr}
\usepackage{amsfonts}
\usepackage[brazilian]{babel}
\usepackage{ifthen}

% \usepackage{tgadventor}
% \renewcommand*\familydefault{\sfdefault} %% Only if the base font of the document is to be sans serif
% \usepackage[T1]{fontenc}
\usepackage[default]{sourcesanspro}
\usepackage[T1]{fontenc}



%Definindo o comando de entrada de itens
\newcommand{\highlight}[1]{
	\textbf{\textcolor{Accent}{#1}}
}

\newcommand{\entry}[6]{
% 1 - Data final
% 2 - Data inicial
% 3 - Cargo
% 4 - Empresa / Instituição
% 5 - Descrição
% 6 - Local

	\noindent \large{#3} \hfill \small{\textcolor{Accent}{#6}} \\
	\textcolor{Accent}{\textbf{\textsc{#4}}} \hfill \textsc{#2} \textcolor{Accent}{-\textrightarrow} \textsc{#1} \\
	\textcolor{Accent}{\vline} \hspace*{0.1cm}
	\begin{minipage}[t]{.75\textwidth}
		\footnotesize{#5}
	\end{minipage}
	\vspace*{.5cm}
}

\newcommand{\skill}[4]{
% 1. Skill
% 2. Proficiency
% 3. Time spent learning
% 4. Description

	\noindent #1 \hfill #3 \\
	\begin{minipage}[t]{.30\textwidth}{\footnotesize\scshape\color{gray}

\ifthenelse{\equal{#2}{1}}{
	\highlight{Iniciante} \hfill Intermediário \hfill Avançado \\
}{}
\ifthenelse{\equal{#2}{2}}{
	Iniciante \hfill \highlight{Intermediário} \hfill Avançado \\
}{}
\ifthenelse{\equal{#2}{3}}{
	Iniciante \hfill Intermediário \hfill \highlight{Avançado} \\
}{}
		}		
		#4
	\end{minipage}

\vspace*{0.5cm}
}

% Espaçamento e formatação de seções
% Estilo de Section
\titleformat{\section}{\Large\scshape\raggedright}{}{0em}{}%
	[
		\vspace*{-0.667\baselineskip} % Espaço vertical entre texto do título e titlerule.
		\textcolor{Accent} % Cor do texto do título.
		{\titlerule} % Adiciona linha abaixo do título.
	]


\titlespacing{\section}{0pt}{0pt}{0pt}

\begin{document}

\pagenumbering{gobble}

\pagestyle{fancy} % tipo de header e footer
\fancyhf{} 
\rfoot{\scriptsize{Atualizado em \today}}
\renewcommand{\headrulewidth}{0pt}

\include{resume.tex}

\end{document}